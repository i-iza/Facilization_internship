 ...
 function(ko, ArrayDataProvider, BufferingDataProvider, ojconverterutils_i18n_1, sharedService) { 
    ...
    CustomerViewModel.prototype._onSaveButtonClick = function() {
      ...
      this.dataprovider.addItem({
        metadata: { key: newCustomerData.name },
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      // Subscribe to data changes to update session storage
      sharedService.subscribe('dataChangeEvent1', (newCustomerData) => {
          viewModel.dataprovider.addItem({
              metadata: { key: newCustomerData.name },
              data: newCustomerData
          });
  
          // Update session storage with the new data
          sessionStorage.setItem('tableData', JSON.stringify(viewModel.customerDataObservableArray()));
      });
  });
