...
function(ko, FilePickerUtils, ArrayDataProvider, BufferingDataProvider) {
  class CustomerViewModel {
    ...
    replaceDialog4 = async () => {
      ...
      try {
        const deleteResponse = await fetch(`http://localhost:8080/deleteFile?filename=${window.filename}`, {
          method: 'DELETE',
          credentials: 'include',
        });
        ...
        if (deleteResponse.ok) {
          ...
          console.error('Failed to delete existing file:', deleteResponse.statusText);
